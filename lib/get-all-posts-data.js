...
// current 'Recetas posts' directory
const postsDirectory = path.join(process.cwd(), 'posts/Recetas');
const mdx_file_extention = '.md';
...
export function getPostMetadata(id) {
	const postMetadata = metadata.filter((metadata, index) => {
		let path = postPaths[index]?.importedPath;
		let path_list = path.split('/');
		path = path_list[path_list.length - 1].replace(/\.md$/, '');
		//console.log(path, id);
		if (path == id)
			return { metadata }
	})
